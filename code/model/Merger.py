...
class Merger_MLP_Elu(nn.Module):
    ...
    def forward(self, a, b):
        ui_dyemb = torch.cat([a, b], dim=-1).cuda(3)
        dyemb = self.elu(self.linear(ui_dyemb)).cuda(3)
        return dyemb.cuda(3)
...
class Merger_DyHGCN(nn.Module):
    ...
    def forward(self, a, b):
        # dynamic_node_emb
        dyemb = self.elu(torch.cat([a, b, a * b, a - b], dim=-1)).cuda(3)
        dyemb = self.elu(self.linear_dyemb(dyemb)).cuda(3)
        return dyemb.cuda(3)
...
class UserMerger(nn.Module):
    ...
    def forward(self, user_embedding, hyper_embedding = None, train = None):
        ...
        Q = (self.linear_1(index).cuda(3))
        K = (self.linear_2(index).cuda(3))
        Q_K = torch.einsum("bld,bmd->bml", Q, K).cuda(3)
        temperature = dim ** 0.5
        episilon = 1e-6
        Q_K = Q_K / (temperature + episilon)
        mask = torch.zeros([max_len, max_len]).cuda(3)
        mask += -2**32+1
        mask = torch.triu(mask, diagonal=0).cuda(3)

        b_mask = torch.zeros_like(Q_K).cuda(3)
        b_mask[:, :, :] = mask[:, :]

        Q_K += b_mask
        score = F.softmax(Q_K, dim=-1).cuda(3)
...
class HiddenMerger(nn.Module):
    ...
    def forward(self, hidden, hyper_embedding = None, train = None):
        ...
        Q = (self.linear_1(hyper_embedding).cuda(3))
        K = (self.linear_2(hyper_embedding).cuda(3))
        Q_K = torch.einsum("bld,bmd->bml", Q, K).cuda(3)
        temperature = dim ** 0.5
        episilon = 1e-6
        Q_K = Q_K / (temperature + episilon)

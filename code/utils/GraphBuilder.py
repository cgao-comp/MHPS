...
def PreprocessByType(data_name, Type):
    ...
    if os.path.exists(option.repost_net_data_trimmed) is False or os.path.exists(option.net_data_refined) is False:
        if data_name == "./data/twitter":
            ...
            BuildTrimedGraphs(data_name, item_threshold=1, threshold=1)
        elif data_name == "./data/android":
            BuildTrimedGraphs(data_name, item_threshold=1, threshold=1)
        elif data_name == "./data/christianity":
            BuildTrimedGraphs(data_name, item_threshold=1, threshold=1)
        elif data_name == "./data/douban_MIDNS":
            BuildTrimedGraphs(data_name, item_threshold=1, threshold=1)
...
def LoadHeteStaticGraph(data_name, Type):
    ...
    if Type.find("diffusion") != -1:
        with open(options.repost_net_data1, 'r') as handle:
            cas_list = handle.read().strip().split("\n")
            cas_list = [edge.split(',') for edge in cas_list]
            relation_list = [(_ui2idx[edge[0]], _ui2idx[edge[1]]) for edge in cas_list if
                             edge[0] in _ui2idx and edge[1] in _ui2idx]
...
def LoadHeteStaticGraph_Hetero(data_name):
    options = Options(data_name)
    _ui2idx = {}
    _idx2ui = []
    ...
    hetero_graph = HeteroData()  # 创建 PyG 异构图

    # 加载 用户-级联事件（user-cascade）交互数据
    with open(options.item_net_data, 'r') as handle:
        relation_list = handle.read().strip().split("\n")
        relation_list = [edge.split(',') for edge in relation_list]
        relation_list = [(edge[0], edge[1]) for edge in relation_list if edge[0] in _ui2idx and edge[1] in _ui2idx]

        edge_index = torch.tensor([(int(_ui2idx[user]), int(_ui2idx[cascade])) for user, cascade in relation_list], dtype=torch.long).t()
        hetero_graph["user", "interacts_with", "cascade"].edge_index = edge_index

    # 加载用户之间的社交关系（social network）
    if os.path.exists(options.net_data_refined):
        with open(options.net_data_refined, 'r') as handle:
            relation_list = handle.read().strip().split("\n")
            relation_list = [edge.split(',') for edge in relation_list]
            relation_list = [(edge[0], edge[1]) for edge in relation_list if edge[0] in _ui2idx and edge[1] in _ui2idx]

            edge_index = torch.tensor([(int(_ui2idx[a]), int(_ui2idx[b])) for a, b in relation_list], dtype=torch.long).t()
            hetero_graph["user", "follows", "user"].edge_index = edge_index

    # 加载用户之间的扩散关系（diffusion process）
    with open(options.repost_net_data, 'r') as handle:
        cas_list = handle.read().strip().split("\n")
        cas_list = [edge.split(',') for edge in cas_list]
        relation_list = [(edge[0], edge[1]) for edge in cas_list if edge[0] in _ui2idx and edge[1] in _ui2idx]

        edge_index = torch.tensor([(int(_ui2idx[a]), int(_ui2idx[b])) for a, b in relation_list], dtype=torch.long).t()
        hetero_graph["user", "reposts", "user"].edge_index = edge_index
